...
### Reuse the same physical database

The September 15 cutover reused the existing Supabase database at the operator's request.
The reviewed [archive operation](archive-test-billing.sql) moves the original 24 billing/credit/
Outbox tables into an administrator-owned `billing_test_YYYYMMDD` schema and creates empty active
replacements with the original constraints, indexes and immutable-ledger triggers. It never deletes
or updates the original financial rows. The application role and Supabase browser roles cannot
access the archive. All new active billing tables have RLS enabled and no browser grants.

Apply `20260915153905_generation_billing_archive` before running that operation. It adds nullable,
display-only settlement amounts to terminal generation jobs. The database accepts the first snapshot
only when it matches the existing settled reservation and prevents later changes. Job history uses
those amounts only when its original reservation has been archived. The snapshots never participate
in spending, reservations, refunds, debt or entitlement decisions. Identities, sessions, assets,
moderation evidence, jobs and edit-history links stay in their original tables.

This is an operator-reviewed data operation, not a Prisma migration to run on every installation.
Before execution:

1. Back up every public application table with PostgreSQL 17 `pg_dump`, using verified TLS. Restore
   the dump into an isolated PostgreSQL 17 database and compare every table's count and content digest.
   Use `COLLATE "C"` in digest ordering so different database locales do not produce false differences.
2. Rehearse the complete transaction and rollback. Check that original financial rows are unchanged,
   active balances are empty, all foreign keys/triggers remain, archived rows are inaccessible to
   `ezpic_app`, and historical job charges still match the original reservations. Include rejection
   of missing evidence, changed data, existing live data, active reservations and repeat execution.
3. Close billing, generation and guest admission on both Workers; drain accepted work and temporarily
   remove the existing scheduler. Preserve its exact schedule and feature-gate values for restoration.
   Do not archive uncertain submissions or pretend an old provider contract has been canceled.
4. Produce a fresh frozen backup and compare its restored contents with the source. Review a manifest
   containing `database`, `archiveSchema`, `admissionAndExecutorDrained`, a verified `backup` receipt
   (`sha256`, `restoreVerified`), `expected` table counts/digests, and the exact 20 production `plans`
   generated by `billing-preflight --manifest`. No credentials belong in this manifest.
5. In a single administrator transaction, set `search_path=public,pg_catalog`, bounded lock/statement
   timeouts and transaction-local `ezpic.cutover_manifest` to that JSON, then execute the archive SQL.
   It locks the application tables, verifies the reviewed fingerprints, preserves terminal-job
   settlement summaries, moves the original tables, restores canonical constraints and inserts the
   new immutable provider snapshots. A mismatch aborts the entire operation. Populated inbound
   foreign-key dependencies require separate review; they are not automatically repointed.
6. Verify the receipt and preserved data, synchronize `.env.production.local`, both runtime Workers
   and both automatic-build configurations to the same live catalog, then restore the scheduler and
   previous generation gates. Run the live preflight and observe authenticated reconciliation using
   the new environment scopes. Keep public collection closed pending merchant approval and live
   payment/refund evidence.

Archive membership does not assert provider closure. Historical test approvals and pending refunds
retain their original observed state in the archive, with the old test credentials in private backup.
Live verifiers use the new application/webhook configuration and reject old-environment signatures.
Do not reconnect an application or scheduler to the archived test tables. After new live activity,
rollback requires a reviewed recovery plan preserving both datasets; do not swap test tables back.

See the [September 15 cutover evidence](evidence/payment-live-cutover-2026-09-15.md) for the applied
database operation, synchronization and remaining merchant/real-payment prerequisites.
