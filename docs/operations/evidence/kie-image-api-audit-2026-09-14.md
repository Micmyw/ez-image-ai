...
- Nano Banana 和 GPT Image 1.5 的输入图片保守限定为 10,000,000 字节。页面选图和生成授权
  同时取模型与套餐上限的较小值，阻止大套餐把过大的图片提交给这两个接口。
...
开启新增三个模型开关后，Cloudflare 报告后台文本绑定数 130 超过 128 上限。
`NEXT_PUBLIC_SITE_NAME`、`NEXT_PUBLIC_SITE_DESCRIPTION` 与 `NEXT_PUBLIC_AVATARS_BUCKET_NAME`
在 Next 构建中使用静态读取，保留构建输入，去掉运行时重复绑定。准备产物增加 128 项上限检查，
当前完整配置生成网站 128 项、后台 127 项。已有同名远程密钥需一次性移除，因为 Wrangler
使用 `--secrets-file` 时仍会继承已有密钥；不删除其他配置或回滚 Worker。
