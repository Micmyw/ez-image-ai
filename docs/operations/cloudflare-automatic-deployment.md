...
新增纯文字生图后的目录为 `2026-09-14.1`。即使模型名称与之前相同，新输入类型和新增
调度路由也不能借用图生图的验收记录。`2026-09-07.2` 与 `2026-09-13.1` 都不能单独
授权该目录；必须先完成新能力的真实验收，再更新两个服务的构建机密。对应的首次发布
拦截和 CI 测试配置遗漏见[9 月 14 日构建排查](evidence/cloudflare-text-catalog-block-2026-09-14.md)。
