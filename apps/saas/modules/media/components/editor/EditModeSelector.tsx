...
export function EditModeSelector({
	...
	const modes = products.flatMap((product) =>
		isEditorProductKey(product.key) ? [{ ...product, key: product.key }] : [],
	);
	...
	return (
		<fieldset className="space-y-3 border-violet-200 bg-violet-50/50 p-4 rounded-xl border">
			...
			<div
				className="gap-3 lg:grid-cols-3 sm:grid-cols-2 grid"
				role="radiogroup"
				aria-label={t("label")}
			>
				{modes.map((mode) => {
			...
			{!allowedProductKeys.includes(value) && (
