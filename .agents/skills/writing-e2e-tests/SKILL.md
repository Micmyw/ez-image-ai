...
description: Use when adding Playwright coverage for a user-visible workflow in the unified SaaS application.
---
...
1. Place product `*.spec.ts` files in `apps/saas/tests`.
2. Read `apps/saas/playwright.config.ts`. Its base URL comes from `NEXT_PUBLIC_SAAS_URL`, defaulting
   to `http://localhost:3000`. The configured web server starts an isolated Webpack development
   server by default, or builds and starts production mode when `E2E_USE_PRODUCTION_BUILD=true`;
...
6. Use the app scripts as intended:
   ...
   ```
   - Full local media harness with isolated seed, Outbox pump, and SaaS draft-handoff scenarios:
   ```bash
   pnpm e2e:media:ci
   ```
...
7. `.github/workflows/validate-prs.yml` provisions PostgreSQL and pinned MinIO, runs the
   immutable-upload regression plus `pnpm e2e:media:ci`, and uploads the SaaS Playwright report and
   test results.
8. Run focused formatting/linting and the affected type checks.

## Canonical reference

`apps/saas/tests/login.spec.ts` uses accessible roles to test auth-mode switching without a
session. `apps/saas/playwright.config.ts` is authoritative for server and artifact behavior.
