...
4. Add matching `pricing.products.<planId>` title, description, and feature keys to every locale's
   `saas.json`. The unified public Pricing route and authenticated UI consume them through
...
8. Run focused payments/API/SaaS tests, formatting/linting checks, and the affected type checks.
...
- Updating authenticated pricing without checking the same-origin public Pricing route.
